...
/************************
createBinTreeWithCopy
this function returns a pointer to a binary tree.  You must send pointers to the compare
 destroy and copy functions when you create the tree.
createBinTreewithCopy returns null if the memory allocation fails.

An example call to this function is: myTree = createBinTree(&compare, &destroyDataNode, &copyData);
*********************/

Tree * createBinTreeWithCopy(int (*comparePointer) (TreeDataTypePtr data1, TreeDataTypePtr data2), void (*destroyPointer) (TreeDataTypePtr), void * (*copyPointer) (TreeDataTypePtr));
...
when destroy is called.   If passed a null pointer it simply returns.
****************************/
...
compare function.  If the data is a duplicate (as indicated by the compare function returning
a zero) then the data will not be added to the tree.
...
It is enough to free the pointer, as this function is making a shallow copy.  Do not call
destroyBinTree on the pointer.  You won't like the results.

returns null if there is no subtree.
*****************************/
...
void printPostOrder(Tree * theTree, void (* printNode) (TreeDataTypePtr data));
